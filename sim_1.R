...
theme_pub <- function(base_size = 10.5, base_family = NULL) {
  theme_bw(base_size = base_size, base_family = base_family) +
    theme(
      ...
      plot.title       = element_text(face = "bold", hjust = 0, size = base_size + 1.5),
...
plot_single_trial <- function(sim_fun,
  ...
  pA <- ggplot(st1$df_stepp, aes(x = x_mid, y = te_hat)) +
    ...
    theme_pub(base_size = plot_base_size)
  ...
  pD <- ggplot(df_np, aes(harm_upper, det)) +
    ...
    annotate(
      ...
      vjust = -1, hjust = 1, size = annotation_size, colour = "red"
    ) +
    scale_x_continuous(labels = label_percent(accuracy = 1)) +
...
run_mc_scenario <- function(sim_fun,
  ...
  for (b in 1:B) {
    ...
    res_list[[b]] <- cbind(data.frame(
      ...
      truth_value_gain_learned = st2$truth_value_gain_learned,
      truth_value_gain_selected_const = st2$truth_value_gain_selected_const,
      selected_const_treat = st2$selected_const_treat,
      learned_treat_fraction = st2$learned_treat_fraction
    ), truth)
  }
...
B_mc <- as.integer(Sys.getenv("SIM_B_MAIN", "500"))
n_mc <- as.integer(Sys.getenv("SIM_N_MAIN", "2000"))

mc_all <- bind_rows(lapply(names(scenario_funs), function(label) {
  run_mc_scenario(
...
write.csv(mc_all, "result/sim_main_SiM.csv", row.names = FALSE)
write.csv(mc_summary, "result/sim_main_SiM_summary.csv", row.names = FALSE)
